...
static bool do_open_port(int argc, char *argv[]);
...
static void console_init()
{
    ...
    add_cmd("open", do_open_port, "                 | Open a uart port");
...
bool do_open_port(int argc, char *argv[])
{
    if (argc != 1) {
        report(1, "%s takes no arguments", argv[0]);
        return false;
    }
    report (1, "%s input argument", argv[0]);
